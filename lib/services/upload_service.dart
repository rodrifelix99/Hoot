...
class UploadService {
  ...
  Future<Uint8List> compressAndCropImage(File file, int size,
    ...
    final croppedImage = square
        ? img.copyResizeCropSquare(image!, size: size)
        : img.copyResize(image!, width: size);
    return img.encodeJpg(croppedImage, quality: 100);
  }

  Future<String> uploadFile(File file, String path,
      {bool compressed = false, int size = 512, bool square = false}) async {
    try {
      final ref = _storage.ref().child('$path/${file.path}');
      if (compressed) {
        final compressedImage =
            await compressAndCropImage(file, size, square: square);
