...
class _CreatePostPageState extends State<CreatePostPage>
  ...
  Future _pickImage() async {
    ...
    if (pickedFile != null &&
        ...
        pickedFile.path.length <= 1000000) {
  ...
  Future _pickGif() async {
    dynamic data = await TenorGifPickerPage.showAsBottomSheet(
      context,
      customCategories:
          _authProvider.user!.feeds!.map((feed) => feed.title).toList() +
              ['Trending'] +
              ['Featured'] +
              ['Reactions'],
      preLoadText: _textEditingController.text.isNotEmpty
  ...
  Widget build(BuildContext context) {
    return Scaffold(
          ...
          : SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  _authProvider.user!.feeds!.isNotEmpty
                      ? Padding(
                              ...
                              dropdownMenuEntries: [
                                for (Feed feed in _authProvider.user!.feeds!)
                                  DropdownMenuEntry(
                                    value: feed.id,
                                    label: feed.title,
                                    trailingIcon: feed.private!
                                        ? const Icon(SolarIconsOutline.lock)
                                        : feed.nsfw!
                                            ? const Icon(SolarIconsOutline.xxx)
                                            : null,
                                  ),
                              ]),
                        )
      ...
      floatingActionButton: !_isLoading && (_images.length + _gifs.length) < 10
